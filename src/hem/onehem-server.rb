...
class HookMap
    ...
    def load
        ...
        hook_pool.each do |hook|
            ...
            id = hook.id
            name = hook.name
            @logger.debug("Hook #{id} (#{name}) type:#{type} #{key} loaded")
        end
...
class HookExecutionManager
    ...
    def execute_action(hook, event)
        ...
        @logger.info("Executing hook #{hook.id} (#{hook.name}) for #{hook.key}")
        ...
        if rc.code.zero?
            @logger.info("Hook #{hook.id} (#{hook.name}) successfully executed for #{hook.key}")
        else
            @logger.error("Failure executing hook #{hook.id} (#{hook.name}) for #{hook.key}")
        end
